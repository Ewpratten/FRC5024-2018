...
void DriveTrain::Drive(XboxController* joystick)
{
	...
	double forwardSpeed = -1 * joystick->GetY(XboxController::kLeftHand);  /* positive is forward */
	double turnAngle    = +1 * joystick->GetX(XboxController::kLeftHand);  /* positive is right */
